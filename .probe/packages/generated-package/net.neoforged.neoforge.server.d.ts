...
import {$EnumArgument} from "net.neoforged.neoforge.server.command.EnumArgument"
import {$ArgumentTypeInfo} from "net.minecraft.commands.synchronization.ArgumentTypeInfo"

export class $EnumArgument$Info$Template implements $ArgumentTypeInfo$Template$$Interface<($EnumArgument<(T)>)> {
public "type"(): $ArgumentTypeInfo<($EnumArgument<(T)>), (never)>
 ...
 * Original type to represent the class type itself. Use in JSDoc only.
 */
export type $EnumArgument$Info$Template$$Original = $EnumArgument$Info$Template;}
